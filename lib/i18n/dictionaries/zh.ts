...
export const zh: Dictionary = {
  ...
  printPage: {
    title: "冲印我的照片",
    intro: "直接从手机上传照片 — 我们会冲印并送到您的酒店，无需前往门店。",
    turnaround: "下午5点前上传：次日送达。下午5点后上传：2天内送达。",
    table: { size: "尺寸", quantity: "数量", price: "价格" },
    sizeLabel: { "4R": "4R（4×6英寸）" },
    quantityLabel: (qty: number) => `${qty} 张`,
    freeLine: (hotelName: string) => `免费 — 由 ${hotelName} 提供`,
    chooseSize: "尺寸",
    chooseQuantity: "需要多少张？",
    printsHeading: "您的照片",
    printsHint: "点击某一格添加1张照片，或制作2张照片拼图。",
    slotEmpty: "添加",
    slotEdit: "编辑",
    slotRemove: "移除",
    slotsReady: (count: number, needed: number) => `已完成 ${count} / ${needed}`,
    continueToReview: "查看订单",
    reviewHeading: "确认您的照片",
    namePlaceholder: "全名",
    phonePlaceholder: "电话号码（含国家代码，例如 +60123456789）",
    emailPlaceholder: "电子邮箱（选填）",
    summaryTotal: (total: number | string) => `总计：RM${total}`,
    submit: "提交订单",
    submitting: "正在提交…",
  },
  collageEditor: {
    chooseLayout: "版式",
    layoutSingle: "1张照片",
    layoutSplitH: "2张照片 — 上下排列",
    layoutSplitV: "2张照片 — 左右排列",
    twoPhotoWarning: "将两张照片放入一张冲印照中，每张照片会变小 — 仍可正常冲印，但近距离查看时人脸或细节可能不太清晰。",
    addPhoto: "添加照片",
    dragHint: "拖动以调整位置",
    usePhoto: "使用此照片",
    cancel: "取消",
    processing: "处理中…",
    compositeFailed: "无法处理此照片，请重试。",
  },
  photoPrintConfirm: {
    title: "订单已收到",
    freeBody: (hotelName: string) => `您的照片正在冲印中，由 ${hotelName} 提供。`,
    paidBody: "已收到付款 — 您的照片正在冲印中。",
    turnaround: "下午5点前上传：次日送达您的酒店。下午5点后上传：2天内送达。",
    back: "返回酒店页面",
    readyTitle: "可供取件",
    readyBody: (slotNumber: number, hotelName: string) => `请于上午10:30前在 ${hotelName} 的 ${slotNumber} 号格取件。`,
    askReception: "如不清楚取件格位置，请询问前台。",
    boxWarning: (time: string) => `24小时内（即 ${time} 前）未取件的照片将被放入木箱中。`,
    movedToBoxTitle: "已移至木箱",
    movedToBoxBody: (hotelName: string) => `您的照片存放在 ${hotelName} 的木箱中，而非编号格位。`,
    cancelledTitle: "订单已取消",
    cancelledBody: "此订单已被取消。",
  },
  photoPrintPay: {
    title: "支付冲印费用",
    summary: (quantity: number, size: string, total: number | string) => `${quantity} 张 ${size} 照片 — 总计 RM${total}`,
    unavailable: "目前无法进行付款。",
  },
  photoPrintServer: {
    hotelUnavailable: "此酒店目前不可用。",
    wrongPhotoCount: (needed: number) => `请上传正好 ${needed} 张照片。`,
    orderFailed: "无法创建您的订单，请重试。",
    alreadyPaid: "此订单已完成付款。",
  },
  booking: {
    stepTitles: {
      locations: "取件与还件地点",
