 ...
 * optional and never gates the customer's own pre-rental check). Camera
...
export async function markHandedOverAction(bookingId: string): Promise<void> {
  ...
  const { data: booking } = await supabase.from("bookings").select("camera_id").eq("id", bookingId).single();
  if (!booking) throw new Error("Booking not found.");

  const { error } = await supabase.from("asset_events").insert({
    asset_type: "CAMERA",
    asset_id: booking.camera_id,
